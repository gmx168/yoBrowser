...
<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1, minimum-scale=0.25"><meta charset="UTF-8"><link rel="icon" href="data:;base64,iVBORw0KGgo="><title>Radio - WEB Board Uploader</title><style>body{background-color:#000;color:#e3d25f;font-size:20px;}
...
<section>
<h2>Maestro - WEB Board Uploader</h2>
<hr />
<span>Select <u>ALL</u> files from <i>Maestro/data/www/</i><br />and upload them using the form below</span>
<hr />
...
</div><div id="progress"><span id="loader"></span></div>
<div id="heap"></div>

<script>
  // Funkcja uruchamiana co 1 sekundę, aby pilnować stopki
  // (Interfejs ładuje się dynamicznie, więc stopka może zniknąć i pojawić się znowu)
  setInterval(function() {
    // Szukamy elementu stopki (zwykle ma id="copy" w yoRadio)
    var footer = document.getElementById("copy");
    
    // Treść Twojej wizytówki
    var myFooter = "Powered by Maestro (V06)/Snawacki as yoRadio MOD | Browser adapted to 0.9.434 by Fryga | Browser by M.B";
    
    if(footer) {
       // Sprawdzamy, czy już nie podmieniliśmy, żeby nie robić tego w kółko
       if(footer.innerHTML !== myFooter) {
          footer.innerHTML = myFooter;
          // Opcjonalnie: zmiana stylu, jeśli tekst jest za długi
